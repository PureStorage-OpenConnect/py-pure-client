...
from pypureclient.flasharray.FA_2_36.models.reference_with_type import ReferenceWithType
...
class ApiClientPost(BaseModel):
    ...
    ApiClientPost
    """
    access_policies: Optional[conlist(ReferenceWithType)] = Field(default=None, description="The access policies allowed for ID Tokens issued by this API client. The bearer of an access token will be authorized to perform actions within the intersection of these policies and those of the array user specified as the JWT `sub` (subject) claim.")
    ...
    max_role: Optional[StrictStr] = Field(default=None, description="Deprecated. The maximum Admin Access Policy (previously 'role') allowed for ID Tokens issued by this API client. The bearer of an access token will be authorized to perform actions within the intersection of this policy and that of the array user specified as the JWT `sub` (subject) claim. `max_role` is deprecated in favor of `access_policies`, but remains for backwards compatibility. If `max_role` is the name of a valid legacy role, it will be interpreted as the corresponding access policy of the same name. Otherwise, it's invalid.")
    ...
    __properties = ["access_policies", "access_token_ttl_in_ms", "issuer", "max_role", "public_key"]
    ...
    def from_dict(cls, obj: dict) -> ApiClientPost:
        ...
        _obj = ApiClientPost.construct(_fields_set=None, **{
            "access_policies": [ReferenceWithType.from_dict(_item) for _item in obj.get("access_policies")] if obj.get("access_policies") is not None else None,
