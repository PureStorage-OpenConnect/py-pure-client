...
class VolumeSnapshotGetResponse(BaseModel):
    ...
    items: Optional[List[VolumeSnapshot]] = Field(default=None, description="Returns a list of all items after filtering. The values are displayed for each name if meaningful. If `total_only=true`, the `items` list will be empty.")
    total: Optional[List[VolumeSnapshot]] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field if meaningful.")
