...
from pypureclient.flasharray.FA_2_33.models.pod_performance_replication import PodPerformanceReplication
...
class PodPerformanceReplicationGetResponse(BaseModel):
    ...
    items: Optional[List[PodPerformanceReplication]] = Field(default=None, description="A list of pod performance replication objects.")
    total: Optional[List[PodPerformanceReplication]] = Field(default=None, description="The aggregate value of all items after filtering. For real-time performance, the values are aggregated for the latest timestamp. For historical performance, the values are aggregated for each timestamp from `start_time` to `end_time`. When applicable, the average value is displayed instead. The values are displayed for each field if meaningful.")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_construct(_fields_set=None, **{
            ...
            "items": [PodPerformanceReplication.from_dict(_item) for _item in obj["items"]] if obj.get("items") is not None else None,
            "total": [PodPerformanceReplication.from_dict(_item) for _item in obj["total"]] if obj.get("total") is not None else None
        })
        return _obj
