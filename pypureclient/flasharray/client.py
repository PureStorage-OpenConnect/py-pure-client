...
def Client(
    ...
    :param model_attribute_error_on_none: Controls model instance behaviour with regard to accessing attributes with None value.
    ...
    array_versions = get_target_versions(configuration=_transport_cfg, target_type='flasharray',
                                            key_to_check='version', timeout=timeout, user_agent=user_agent)
    if version is not None:
        version = __validate_version(array_versions, version)
    else:
        version = __choose_version(array_versions)

    fa_module = __version_to_module(version)
