...
class PodSpace(BaseModel):
    ...
    total_physical: Optional[conint(strict=True, ge=0)] = Field(default=None, description="This field has been deprecated. Please use the `total_used` field, as it contains the same information.")
    ...
    used_provisioned: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The amount of logical space a container has consumed. The amount of logical space is compared against the quota limit if the container has one configured. Used provisioned does not include destroyed objects inside the container. For a destroyed container, used provisioned can include destroyed objects and represents how much logical space it would take to recover the container.")
    ...
    __properties = ["data_reduction", "shared", "snapshots", "system", "thin_provisioning", "total_physical", "total_provisioned", "total_reduction", "total_used", "unique", "used_provisioned", "virtual", "replication", "replication_effective", "shared_effective"]
