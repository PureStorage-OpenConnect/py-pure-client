...
class PodSpace(BaseModel):
    ...
    replication_effective: Optional[conint(strict=True, ge=0)] = Field(default=None, description="Deprecated. The effective space consumed by unique data that has not been replicated to the target pod, on source pods. The effective space consumed by unique data that has been replicated to the target but not yet incorporated into the target pod, on target pods. Please use the `replication` field in the future, as it contains the same information for Evergreen//One arrays.")
    shared_effective: Optional[conint(strict=True, ge=0)] = Field(default=None, description="Deprecated. The effective space contributed by data that is not unique to a specific volume, managed directory, or snapshot, measured in bytes. Please use the `shared` field in the future, as it contains the same information for Evergreen//One arrays.")
    __properties = ["data_reduction", "shared", "snapshots", "system", "thin_provisioning", "total_physical", "total_provisioned", "total_reduction", "unique", "used_provisioned", "virtual", "replication", "replication_effective", "shared_effective"]
