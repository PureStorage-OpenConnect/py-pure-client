...
from pypureclient.flasharray.common.models.resource_directory_space_v_50 import ResourceDirectorySpace


class ResourceDirectorySpaceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourceDirectorySpace)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourceDirectorySpace)] = Field(default=None, description="The aggregate value of all items after filtering. Where meaningful, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourceDirectorySpaceGetResponse:
        """Create an instance of ResourceDirectorySpaceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourceDirectorySpaceGetResponse:
        """Create an instance of ResourceDirectorySpaceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourceDirectorySpaceGetResponse.parse_obj(obj)

        _obj = ResourceDirectorySpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourceDirectorySpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourceDirectorySpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
