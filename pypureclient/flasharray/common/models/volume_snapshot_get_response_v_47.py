...
from pypureclient.flasharray.common.models.snapshot_v_0 import Snapshot
from pypureclient.flasharray.common.models.snapshot_space_v_34 import SnapshotSpace
...
from pypureclient.flasharray.common.models.volume_snapshot_v_47 import VolumeSnapshot


class VolumeSnapshotGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(VolumeSnapshot)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    ...
    total: Optional[conlist(VolumeSnapshot)] = Field(default=None, description="The aggregate value of all items after filtering. Where meaningful, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VolumeSnapshotGetResponse:
        """Create an instance of VolumeSnapshotGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> VolumeSnapshotGetResponse:
        """Create an instance of VolumeSnapshotGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VolumeSnapshotGetResponse.parse_obj(obj)

        _obj = VolumeSnapshotGetResponse.construct(_fields_set=None, **{
            ...
            "items": [VolumeSnapshot.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [VolumeSnapshot.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
