...
from pypureclient.flasharray.common.models.pod_member_v_49 import PodMember


class PodMemberGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PodMember)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PodMemberGetResponse:
        """Create an instance of PodMemberGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PodMemberGetResponse:
        """Create an instance of PodMemberGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PodMemberGetResponse.parse_obj(obj)

        _obj = PodMemberGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PodMember.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
