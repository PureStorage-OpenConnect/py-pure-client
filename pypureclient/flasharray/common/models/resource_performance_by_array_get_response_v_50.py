...
from pypureclient.flasharray.common.models.resource_performance_v_50 import ResourcePerformance
...
from pypureclient.flasharray.common.models.resource_performance_by_array_v_50 import ResourcePerformanceByArray


class ResourcePerformanceByArrayGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePerformanceByArray)] = Field(default=None, description="Performance data, broken down by array. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePerformanceByArray)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    __properties = ["continuation_token", "more_items_remaining", "total_item_count", "errors", "items", "total"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePerformanceByArrayGetResponse:
        """Create an instance of ResourcePerformanceByArrayGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePerformanceByArrayGetResponse:
        """Create an instance of ResourcePerformanceByArrayGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePerformanceByArrayGetResponse.parse_obj(obj)

        _obj = ResourcePerformanceByArrayGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourcePerformanceByArray.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePerformanceByArray.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
