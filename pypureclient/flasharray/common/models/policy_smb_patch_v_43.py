...
from pypureclient.flasharray.common.models.new_name_v_0 import NewName
...
class PolicySmbPatch(BaseModel):
    """
    PolicySmbPatch
    """
    ...
    continuous_availability_enabled: Optional[StrictBool] = Field(default=None, description="If set to `true`, enables continuous availability on the policy. When continuous availability is enabled on a policy, file shares are accessible during otherwise disruptive scenarios such as temporary network outages, controller upgrades or failovers.")
    __properties = ["name", "enabled", "access_based_enumeration_enabled", "continuous_availability_enabled"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicySmbPatch:
        """Create an instance of PolicySmbPatch from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicySmbPatch:
        """Create an instance of PolicySmbPatch from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicySmbPatch.parse_obj(obj)

        _obj = PolicySmbPatch.construct(_fields_set=None, **{
            "name": obj.get("name"),
