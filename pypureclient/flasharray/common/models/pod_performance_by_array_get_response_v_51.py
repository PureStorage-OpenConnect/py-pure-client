...
from pypureclient.flasharray.common.models.pod_performance_v_51 import PodPerformance
...
from pypureclient.flasharray.common.models.performance_v_51 import Performance
...
from pypureclient.flasharray.common.models.pod_performance_by_array_v_51 import PodPerformanceByArray


class PodPerformanceByArrayGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PodPerformanceByArray)] = Field(default=None, description="Performance data, broken down by array. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(PodPerformanceByArray)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PodPerformanceByArrayGetResponse:
        """Create an instance of PodPerformanceByArrayGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PodPerformanceByArrayGetResponse:
        """Create an instance of PodPerformanceByArrayGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PodPerformanceByArrayGetResponse.parse_obj(obj)

        _obj = PodPerformanceByArrayGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PodPerformanceByArray.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [PodPerformanceByArray.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
