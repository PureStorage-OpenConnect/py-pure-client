...
from pypureclient.flasharray.common.models.protection_group_snapshot_v_47 import ProtectionGroupSnapshot


class ProtectionGroupSnapshotGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ProtectionGroupSnapshot)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    total: Optional[conlist(ProtectionGroupSnapshot)] = Field(default=None, description="The aggregate value of all items after filtering. If applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProtectionGroupSnapshotGetResponse:
        """Create an instance of ProtectionGroupSnapshotGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ProtectionGroupSnapshotGetResponse:
        """Create an instance of ProtectionGroupSnapshotGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProtectionGroupSnapshotGetResponse.parse_obj(obj)

        _obj = ProtectionGroupSnapshotGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ProtectionGroupSnapshot.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [ProtectionGroupSnapshot.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
