...
from pypureclient.flasharray.common.models.directory_performance_v_50 import DirectoryPerformance
from pypureclient.flasharray.common.models.errorcontextresponse_errors_v_49 import ErrorcontextresponseErrors


class DirectoryPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(DirectoryPerformance)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(DirectoryPerformance)] = Field(default=None, description="The aggregate value of all items after filtering. When it makes sense, the average value is displayed instead. The values are displayed for each field if meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DirectoryPerformanceGetResponse:
        """Create an instance of DirectoryPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DirectoryPerformanceGetResponse:
        """Create an instance of DirectoryPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DirectoryPerformanceGetResponse.parse_obj(obj)

        _obj = DirectoryPerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [DirectoryPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [DirectoryPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
