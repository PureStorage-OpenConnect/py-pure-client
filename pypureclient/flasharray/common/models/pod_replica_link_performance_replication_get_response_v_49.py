...
from pypureclient.flasharray.common.models.replica_link_performance_replication_v_49 import ReplicaLinkPerformanceReplication
...
from pypureclient.flasharray.common.models.pod_replica_link_performance_replication_v_49 import PodReplicaLinkPerformanceReplication


class PodReplicaLinkPerformanceReplicationGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PodReplicaLinkPerformanceReplication)] = Field(default=None, description="A list of pod replica link performance objects.")
    total: Optional[conlist(PodReplicaLinkPerformanceReplication)] = Field(default=None, description="The aggregate value of all items after filtering. For real-time performance, the values are aggregated for the latest timestamp. For historical performance, the values are aggregated for each timestamp from `start_time` to `end_time`. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PodReplicaLinkPerformanceReplicationGetResponse:
        """Create an instance of PodReplicaLinkPerformanceReplicationGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PodReplicaLinkPerformanceReplicationGetResponse:
        """Create an instance of PodReplicaLinkPerformanceReplicationGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PodReplicaLinkPerformanceReplicationGetResponse.parse_obj(obj)

        _obj = PodReplicaLinkPerformanceReplicationGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PodReplicaLinkPerformanceReplication.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [PodReplicaLinkPerformanceReplication.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None,
