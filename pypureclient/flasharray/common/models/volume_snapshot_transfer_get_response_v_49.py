...
from pypureclient.flasharray.common.models.transfer_v_1 import Transfer
...
from pypureclient.flasharray.common.models.volume_snapshot_transfer_v_49 import VolumeSnapshotTransfer


class VolumeSnapshotTransferGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(VolumeSnapshotTransfer)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty.")
    ...
    total: Optional[conlist(VolumeSnapshotTransfer)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VolumeSnapshotTransferGetResponse:
        """Create an instance of VolumeSnapshotTransferGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> VolumeSnapshotTransferGetResponse:
        """Create an instance of VolumeSnapshotTransferGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VolumeSnapshotTransferGetResponse.parse_obj(obj)

        _obj = VolumeSnapshotTransferGetResponse.construct(_fields_set=None, **{
            ...
            "items": [VolumeSnapshotTransfer.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [VolumeSnapshotTransfer.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
