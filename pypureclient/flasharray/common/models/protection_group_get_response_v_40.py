...
from pypureclient.flasharray.common.models.retention_policy_v_1 import RetentionPolicy
...
from pypureclient.flasharray.common.models.workload_configuration_fixed_reference_v_40 import WorkloadConfigurationFixedReference
from pypureclient.flasharray.common.models.workload_configuration_fixed_reference_workload_v_40 import WorkloadConfigurationFixedReferenceWorkload
...
from pypureclient.flasharray.common.models.errorcontextresponse_errors_v_38 import ErrorcontextresponseErrors
from pypureclient.flasharray.common.models.protection_group_v_40 import ProtectionGroup


class ProtectionGroupGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ProtectionGroup)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    ...
    total: Optional[conlist(ProtectionGroup)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProtectionGroupGetResponse:
        """Create an instance of ProtectionGroupGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ProtectionGroupGetResponse:
        """Create an instance of ProtectionGroupGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProtectionGroupGetResponse.parse_obj(obj)

        _obj = ProtectionGroupGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ProtectionGroup.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [ProtectionGroup.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
