...
from pypureclient.flasharray.common.models.directory_snapshot_v_42 import DirectorySnapshot
from pypureclient.flasharray.common.models.errorcontextresponse_errors_v_38 import ErrorcontextresponseErrors


class DirectorySnapshotGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(DirectorySnapshot)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    total: Optional[conlist(DirectorySnapshot)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DirectorySnapshotGetResponse:
        """Create an instance of DirectorySnapshotGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DirectorySnapshotGetResponse:
        """Create an instance of DirectorySnapshotGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DirectorySnapshotGetResponse.parse_obj(obj)

        _obj = DirectorySnapshotGetResponse.construct(_fields_set=None, **{
            ...
            "items": [DirectorySnapshot.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [DirectorySnapshot.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
