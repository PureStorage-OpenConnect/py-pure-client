...
from pypureclient.flasharray.common.models.local_member_v_47 import LocalMember
...
class LocalMembershipGetResponse(BaseModel):
    ...
    items: Optional[conlist(LocalMember)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name, if meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> LocalMembershipGetResponse:
        """Create an instance of LocalMembershipGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> LocalMembershipGetResponse:
        """Create an instance of LocalMembershipGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return LocalMembershipGetResponse.parse_obj(obj)

        _obj = LocalMembershipGetResponse.construct(_fields_set=None, **{
            ...
            "items": [LocalMember.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
