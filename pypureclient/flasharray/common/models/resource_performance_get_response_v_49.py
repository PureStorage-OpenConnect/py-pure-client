...
from pypureclient.flasharray.common.models.resource_performance_v_49 import ResourcePerformance


class ResourcePerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePerformance)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePerformance)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePerformanceGetResponse:
        """Create an instance of ResourcePerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePerformanceGetResponse:
        """Create an instance of ResourcePerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePerformanceGetResponse.parse_obj(obj)

        _obj = ResourcePerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourcePerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
