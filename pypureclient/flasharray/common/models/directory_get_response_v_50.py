...
from pypureclient.flasharray.common.models.limited_by_v_49 import LimitedBy
...
from pypureclient.flasharray.common.models.directory_v_50 import Directory
...
class DirectoryGetResponse(BaseModel):
    ...
    items: Optional[conlist(Directory)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    total: Optional[conlist(Directory)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DirectoryGetResponse:
        """Create an instance of DirectoryGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DirectoryGetResponse:
        """Create an instance of DirectoryGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DirectoryGetResponse.parse_obj(obj)

        _obj = DirectoryGetResponse.construct(_fields_set=None, **{
            ...
            "items": [Directory.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [Directory.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
