...
from pypureclient.flasharray.common.models.resource_performance_no_id_v_38 import ResourcePerformanceNoId
...
from pypureclient.flasharray.common.models.resource_performance_no_id_by_array_v_38 import ResourcePerformanceNoIdByArray


class ResourcePerformanceNoIdByArrayGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePerformanceNoIdByArray)] = Field(default=None, description="Performance data, broken down by array. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePerformanceNoIdByArray)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePerformanceNoIdByArrayGetResponse:
        """Create an instance of ResourcePerformanceNoIdByArrayGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePerformanceNoIdByArrayGetResponse:
        """Create an instance of ResourcePerformanceNoIdByArrayGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePerformanceNoIdByArrayGetResponse.parse_obj(obj)

        _obj = ResourcePerformanceNoIdByArrayGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourcePerformanceNoIdByArray.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePerformanceNoIdByArray.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
