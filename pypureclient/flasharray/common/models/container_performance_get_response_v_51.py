...
from pypureclient.flasharray.common.models.container_performance_v_51 import ContainerPerformance
from pypureclient.flasharray.common.models.errorcontextresponse_errors_v_49 import ErrorcontextresponseErrors


class ContainerPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ContainerPerformance)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ContainerPerformance)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ContainerPerformanceGetResponse:
        """Create an instance of ContainerPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ContainerPerformanceGetResponse:
        """Create an instance of ContainerPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ContainerPerformanceGetResponse.parse_obj(obj)

        _obj = ContainerPerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ContainerPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ContainerPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
