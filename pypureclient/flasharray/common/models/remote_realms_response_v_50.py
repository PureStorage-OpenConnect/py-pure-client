...
from pypureclient.flasharray.common.models.remote_realm_v_50 import RemoteRealm
...
class RemoteRealmsResponse(BaseModel):
    ...
    items: Optional[conlist(RemoteRealm)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RemoteRealmsResponse:
        """Create an instance of RemoteRealmsResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RemoteRealmsResponse:
        """Create an instance of RemoteRealmsResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RemoteRealmsResponse.parse_obj(obj)

        _obj = RemoteRealmsResponse.construct(_fields_set=None, **{
            ...
            "items": [RemoteRealm.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
