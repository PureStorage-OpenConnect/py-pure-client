...
from pypureclient.flasharray.common.models.resource_performance_no_id_v_50 import ResourcePerformanceNoId


class ResourcePerformanceNoIdGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePerformanceNoId)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePerformanceNoId)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePerformanceNoIdGetResponse:
        """Create an instance of ResourcePerformanceNoIdGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePerformanceNoIdGetResponse:
        """Create an instance of ResourcePerformanceNoIdGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePerformanceNoIdGetResponse.parse_obj(obj)

        _obj = ResourcePerformanceNoIdGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourcePerformanceNoId.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePerformanceNoId.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
