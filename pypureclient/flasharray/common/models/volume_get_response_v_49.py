...
from pypureclient.flasharray.common.models.priority_adjustment_v_10 import PriorityAdjustment
from pypureclient.flasharray.common.models.protocol_endpoint_v_33 import ProtocolEndpoint
from pypureclient.flasharray.common.models.qos_v_0 import Qos
...
from pypureclient.flasharray.common.models.volume_common_v_49 import VolumeCommon
from pypureclient.flasharray.common.models.volume_space_common_v_34 import VolumeSpaceCommon
from pypureclient.flasharray.common.models.workload_configuration_reference_v_40 import WorkloadConfigurationReference
...
from pypureclient.flasharray.common.models.volume_v_49 import Volume
...
class VolumeGetResponse(BaseModel):
    ...
    items: Optional[conlist(Volume)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list is empty.")
    ...
    total: Optional[conlist(Volume)] = Field(default=None, description="The aggregate value of all items after filtering. If applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    __properties = ["continuation_token", "more_items_remaining", "total_item_count", "items", "errors", "total"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VolumeGetResponse:
        """Create an instance of VolumeGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> VolumeGetResponse:
        """Create an instance of VolumeGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VolumeGetResponse.parse_obj(obj)

        _obj = VolumeGetResponse.construct(_fields_set=None, **{
            ...
            "items": [Volume.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [Volume.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
