...
from pypureclient.flasharray.common.models.server_v_50 import Server
...
class ServerGetResponse(BaseModel):
    ...
    items: Optional[conlist(Server)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. The values are displayed for each field, if meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ServerGetResponse:
        """Create an instance of ServerGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ServerGetResponse:
        """Create an instance of ServerGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ServerGetResponse.parse_obj(obj)

        _obj = ServerGetResponse.construct(_fields_set=None, **{
            ...
            "items": [Server.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
