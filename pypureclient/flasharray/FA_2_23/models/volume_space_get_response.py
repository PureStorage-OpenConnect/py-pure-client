...
from pypureclient.flasharray.FA_2_23.models.volume_space import VolumeSpace
...
class VolumeSpaceGetResponse(BaseModel):
    ...
    items: Optional[conlist(VolumeSpace)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name, if applicable. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(VolumeSpace)] = Field(default=None, description="The aggregate value of all items after filtering. If applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
