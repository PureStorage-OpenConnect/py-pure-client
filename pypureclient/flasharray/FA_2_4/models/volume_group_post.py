...
from pypureclient.flasharray.FA_2_4.models.qos import Qos
from pypureclient.flasharray.FA_2_4.models.space import Space
...
class VolumeGroupPost(BaseModel):
    ...
    destroyed: Optional[StrictBool] = Field(default=None, description="Has this volume group been destroyed? To destroy a volume group, patch to `true`. To recover a destroyed volume group, patch to `false`. If not specified, defaults to `false`.")
    qos: Optional[Qos] = None
    space: Optional[Space] = None
    time_remaining: Optional[StrictInt] = Field(default=None, description="The amount of time left until the destroyed volume group is permanently eradicated. Measured in milliseconds. Before the `time_remaining` period has elapsed, the destroyed volume group can be recovered by setting `destroyed=false`.")
    volume_count: Optional[StrictInt] = Field(default=None, description="The number of volumes in the volume group.")
    __properties = ["id", "name", "destroyed", "qos", "space", "time_remaining", "volume_count"]
