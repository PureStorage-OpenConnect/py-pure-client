...
from pypureclient.flasharray.FA_2_4.models.volume_snapshot import VolumeSnapshot
...
class VolumeSnapshotGetResponse(BaseModel):
    ...
    items: Optional[conlist(VolumeSnapshot)] = Field(default=None, description="Returns a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(VolumeSnapshot)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
