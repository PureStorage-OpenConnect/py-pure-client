...
from pypureclient.flasharray.FA_2_16.models.pod_performance import PodPerformance


class PodPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PodPerformance)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(PodPerformance)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PodPerformanceGetResponse:
        """Create an instance of PodPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PodPerformanceGetResponse:
        """Create an instance of PodPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PodPerformanceGetResponse.parse_obj(obj)

        _obj = PodPerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PodPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [PodPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
