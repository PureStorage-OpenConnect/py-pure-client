...
from pypureclient.flasharray._common.models.resource_realm_space_v_2_52 import ResourceRealmSpace


class RealmSpaceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourceRealmSpace)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourceRealmSpace)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. The values are displayed for each field, if meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RealmSpaceGetResponse:
        """Create an instance of RealmSpaceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RealmSpaceGetResponse:
        """Create an instance of RealmSpaceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RealmSpaceGetResponse.parse_obj(obj)

        _obj = RealmSpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourceRealmSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourceRealmSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
