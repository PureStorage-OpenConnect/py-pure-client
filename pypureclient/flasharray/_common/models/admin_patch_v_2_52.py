...
class AdminPatch(BaseModel):
    ...
    admin_type: Optional[StrictStr] = Field(default=None, description="The type of administrative account. A value of `local-user` indicates the account is managed directly on the array, while `remote-user` indicates the account is provisioned by an external system such as `LDAP` or `SAML`.")
    api_token: Optional[ApiToken] = None
    authorization_model: Optional[StrictStr] = Field(default=None, description="The source of the access policy mapping. A value of `static` indicates that policies are configured locally on the array, while a value of `dynamic` indicates that they are determined by an external system such as `LDAP` or an `IdP`.")
    ...
    __properties = ["name", "admin_type", "api_token", "authorization_model", "is_local", "locked", "lockout_remaining", "management_access_policies", "password", "public_key", "role", "old_password"]
