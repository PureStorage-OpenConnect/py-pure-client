...
from pypureclient.flasharray._common.models.policyrulemanagementaccess_rules_v_2_49 import PolicyrulemanagementaccessRules
...
class PolicyManagementAccess(BaseModel):
    ...
    aggregation_strategy: Optional[StrictStr] = Field(default=None, description="When this is set to `least-common-permissions`, any users to whom this policy applies can receive no access rights exceeding those defined in this policy's rules. When this is set to `all-permissions`, any users to whom this policy applies are capable of receiving additional access rights from other policies that apply to them.")
    ...
    rules: Optional[conlist(PolicyrulemanagementaccessRules)] = Field(default=None, description="List the rules that defines the scoped roles.")
    __properties = ["id", "name", "context", "aggregation_strategy", "enabled", "policy_type", "rules"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicyManagementAccess:
        """Create an instance of PolicyManagementAccess from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicyManagementAccess:
        """Create an instance of PolicyManagementAccess from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicyManagementAccess.parse_obj(obj)

        _obj = PolicyManagementAccess.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
            ...
            "rules": [PolicyrulemanagementaccessRules.from_dict(_item) for _item in obj.get("rules")] if obj.get("rules") is not None else None
        })
        return _obj
