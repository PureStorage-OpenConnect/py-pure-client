...
from pypureclient.flasharray._common.models.fleet_v_2_51 import Fleet


class FleetGetResponse(BaseModel):
    """
    FleetGetResponse
    """
    items: Optional[conlist(Fleet)] = Field(default=None, description="Returns a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    __properties = ["items", "continuation_token", "more_items_remaining", "total_item_count"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FleetGetResponse:
        """Create an instance of FleetGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FleetGetResponse:
        """Create an instance of FleetGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FleetGetResponse.parse_obj(obj)

        _obj = FleetGetResponse.construct(_fields_set=None, **{
            "items": [Fleet.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
