...
from pypureclient.flasharray._common.models.realm_member_v_2_55 import RealmMember


class RealmMemberGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(RealmMember)] = Field(default=None, description="The list of all `items` displayed after `filtering`. The `values` are displayed for each `name` where meaningful. If the `total_only` parameter is set to `true`, the `items` list will be empty.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RealmMemberGetResponse:
        """Create an instance of RealmMemberGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RealmMemberGetResponse:
        """Create an instance of RealmMemberGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RealmMemberGetResponse.parse_obj(obj)

        _obj = RealmMemberGetResponse.construct(_fields_set=None, **{
            ...
            "items": [RealmMember.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
