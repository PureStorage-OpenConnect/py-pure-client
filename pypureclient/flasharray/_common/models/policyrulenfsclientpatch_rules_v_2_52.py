...
class PolicyrulenfsclientpatchRules(BaseModel):
    """
    PolicyrulenfsclientpatchRules
    """
    access: Optional[StrictStr] = Field(default=None, description="Specifies access control for the export. Values include `root-squash`, `all-squash`, and `no-root-squash`. The `root-squash` value prevents client users and groups with root privilege from mapping their root privilege to a file system. All users with UID `0` will have their UID mapped to `anonuid`. All users with GID `0` will have their GID mapped to `anongid`. The `all-squash` value maps all UIDs (including root) to `anonuid` and all GIDs (including root) to `anongid`. The `no-root-squash` value allows users and groups to access the file system with their UIDs and GIDs.")
    anongid: Optional[StrictStr] = Field(default=None, description="Any user whose GID is affected by an `access` of `root_squash` or `all_squash` will have their GID mapped to `anongid`. If not specified, defaults to `65534`. Set to `\"\"` to clear. This value is ignored when user mapping is enabled.")
    ...
    nfs_version: Optional[conlist(StrictStr)] = Field(default=None, description="The NFS protocol version allowed for the export. Values include `nfsv3` and `nfsv4`.")
    permission: Optional[StrictStr] = Field(default=None, description="Specifies which read-write client access permissions are allowed for the export. Values include `rw` and `ro`.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicyrulenfsclientpatchRules:
        """Create an instance of PolicyrulenfsclientpatchRules from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicyrulenfsclientpatchRules:
        """Create an instance of PolicyrulenfsclientpatchRules from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicyrulenfsclientpatchRules.parse_obj(obj)

        _obj = PolicyrulenfsclientpatchRules.construct(_fields_set=None, **{
