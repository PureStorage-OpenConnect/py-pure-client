...
from pypureclient.flasharray._common.models.vchost_endpoint_v_2_52 import VchostEndpoint


class VchostEndpointGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(VchostEndpoint)] = Field(default=None, description="Returns a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VchostEndpointGetResponse:
        """Create an instance of VchostEndpointGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> VchostEndpointGetResponse:
        """Create an instance of VchostEndpointGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VchostEndpointGetResponse.parse_obj(obj)

        _obj = VchostEndpointGetResponse.construct(_fields_set=None, **{
            ...
            "items": [VchostEndpoint.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
