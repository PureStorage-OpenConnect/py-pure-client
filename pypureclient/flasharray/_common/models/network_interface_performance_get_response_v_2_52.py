...
from pypureclient.flasharray._common.models.network_interface_performance_v_2_52 import NetworkInterfacePerformance


class NetworkInterfacePerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(NetworkInterfacePerformance)] = Field(default=None, description="A list of performance objects.")
    total: Optional[conlist(NetworkInterfacePerformance)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. The values are displayed for each field where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NetworkInterfacePerformanceGetResponse:
        """Create an instance of NetworkInterfacePerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NetworkInterfacePerformanceGetResponse:
        """Create an instance of NetworkInterfacePerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NetworkInterfacePerformanceGetResponse.parse_obj(obj)

        _obj = NetworkInterfacePerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [NetworkInterfacePerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [NetworkInterfacePerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
