...
from pypureclient.flasharray._common.models.workload_placement_recommendation_resource_reference_v_2_54 import WorkloadPlacementRecommendationResourceReference


class WorkloadPlacementRecommendationAdditionalConstraints(BaseModel):
    """
    The list of targets considered for workload placement must satisfy the requirements imposed by the `preset`. Users can also specify additional `constraints` to further restrict the list of targets for consideration.
    """
    required_resource_references: Optional[conlist(WorkloadPlacementRecommendationResourceReference)] = Field(default=None, description="The property that defines a group of resources. At least one resource from each item must be present on the `target` for it to be considered an acceptable candidate for workload placement.")
    targets: Optional[conlist(ReferenceWithType)] = Field(default=None, description="The list of `targets` to consider when computing placement recommendations. If not specified, the entire fleet is used to compute the recommendation. Specify `targets` as individual arrays or as topology groups by setting `name` to `[group].arrays`. The resulting response contains `targets` represented as individual arrays.")
