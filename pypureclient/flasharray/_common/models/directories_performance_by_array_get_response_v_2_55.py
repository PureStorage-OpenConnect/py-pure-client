...
from pypureclient.flasharray._common.models.directories_performance_by_array_v_2_55 import DirectoriesPerformanceByArray
from pypureclient.flasharray._common.models.errorcontextresponse_errors_v_2_49 import ErrorcontextresponseErrors


class DirectoriesPerformanceByArrayGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(DirectoriesPerformanceByArray)] = Field(default=None, description="The `performance data` broken down by `array`. If the `total_only` parameter is set to `true`, the `items` list will be empty.")
    total: Optional[conlist(DirectoriesPerformanceByArray)] = Field(default=None, description="The aggregate value of all `items` after `filtering`. When applicable, the `average value` is displayed instead. If applicable, the `values` are displayed for each `field`.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DirectoriesPerformanceByArrayGetResponse:
        """Create an instance of DirectoriesPerformanceByArrayGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DirectoriesPerformanceByArrayGetResponse:
        """Create an instance of DirectoriesPerformanceByArrayGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DirectoriesPerformanceByArrayGetResponse.parse_obj(obj)

        _obj = DirectoriesPerformanceByArrayGetResponse.construct(_fields_set=None, **{
            ...
            "items": [DirectoriesPerformanceByArray.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [DirectoriesPerformanceByArray.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
