...
from pypureclient.flasharray._common.models.qos_metrics_ceilings_v_2_52 import QosMetricsCeilings


class HostGroupQosGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(QosMetricsCeilings)] = Field(default=None, description="Specifies the Quality of Service (QoS) configuration data.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> HostGroupQosGetResponse:
        """Create an instance of HostGroupQosGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> HostGroupQosGetResponse:
        """Create an instance of HostGroupQosGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HostGroupQosGetResponse.parse_obj(obj)

        _obj = HostGroupQosGetResponse.construct(_fields_set=None, **{
            ...
            "items": [QosMetricsCeilings.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
