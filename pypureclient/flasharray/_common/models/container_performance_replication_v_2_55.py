...
class ContainerPerformanceReplication(BaseModel):
    """
    ...
    continuous_bytes_per_sec: Optional[ReplicationPerformanceWithTotal] = Field(default=None, description="The total number of `bytes` transmitted or received per second for continuous replication. The continuous replication feature is used for disaster recovery on FlashArray and provides a recovery point objective (RPO) of significantly less than 30 seconds.")
    periodic_bytes_per_sec: Optional[ReplicationPerformanceWithTotal] = Field(default=None, description="The total number of `bytes` transmitted or received per second for periodic replication.")
    resync_bytes_per_sec: Optional[ReplicationPerformanceWithTotal] = Field(default=None, description="The total number of `bytes` transmitted or received per second during resync replication. Resync replication is the mechanism to bring two `arrays` into sync. This may occur during an initial `pod` stretch, or, in the case of an outage, when two `arrays` reestablish connection. After the connection is restored, the `array` that was online starts replicating `pod` data to its peer `array` until the `pod` is once again in sync.")
    sync_bytes_per_sec: Optional[ReplicationPerformanceWithTotal] = Field(default=None, description="The total number of `bytes` transmitted or received per second for synchronous replication.")
    time: Optional[StrictInt] = Field(default=None, description="The sample time recorded in `milliseconds` since the UNIX epoch.")
    total_bytes_per_sec: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The total number of `bytes` transmitted and received per second for all types of replication.")
    __properties = ["context", "continuous_bytes_per_sec", "periodic_bytes_per_sec", "resync_bytes_per_sec", "sync_bytes_per_sec", "time", "total_bytes_per_sec"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ContainerPerformanceReplication:
        """Create an instance of ContainerPerformanceReplication from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ContainerPerformanceReplication:
        """Create an instance of ContainerPerformanceReplication from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ContainerPerformanceReplication.parse_obj(obj)

        _obj = ContainerPerformanceReplication.construct(_fields_set=None, **{
