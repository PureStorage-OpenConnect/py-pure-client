...
QUOTALIMITPATCH_ONE_OF_SCHEMAS = ["int", "str"]

class QuotaLimitPatch(BaseModel):
    """
    The logical quota limit of the object, measured in bytes. In order to unset use \"\".
    """
    # data type: int
    oneof_schema_1_validator: Optional[conint(strict=True, le=4503599627370496, ge=1048576)] = None
    # data type: str
    ...
    if TYPE_CHECKING:
        ...
        actual_instance: Any
    one_of_schemas: List[str] = Field(QUOTALIMITPATCH_ONE_OF_SCHEMAS, const=True)
    ...
    def validate(cls, value):
        ...
        Pydantic calls this hook whenever QuotaLimitPatch is used as a field type
        ...
        instead of the strict form:
            ...
            model.field = QuotaLimitPatch("raw-string-value")

        This hook coerces all of those into a QuotaLimitPatch so the
        `actual_instance` oneOf validator still runs against the wrapped value.
        """
        # Already a QuotaLimitPatch: pass through untouched.
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = QuotaLimitPatch.construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in QuotaLimitPatch with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in QuotaLimitPatch with oneOf schemas: int, str. Details: " + ", ".join(error_messages))
        else:
            return v

    @classmethod
    def from_dict(cls, obj: dict) -> QuotaLimitPatch:
        return cls.from_json(json.dumps(obj))

    @classmethod
    def from_json(cls, json_str: str) -> QuotaLimitPatch:
        """Returns the object represented by the json string"""
        return QuotaLimitPatch.construct(_fields_set=None, **{
