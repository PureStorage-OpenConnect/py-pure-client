...
from pypureclient.flasharray._common.models.policy_qos_v_2_54 import PolicyQos


class PolicyQosGetResponse(BaseModel):
    """
    PolicyQosGetResponse
    """
    ...
    items: Optional[conlist(PolicyQos)] = Field(default=None, description="The operation that displays a list of all items after filtering. If applicable, the specific values are displayed for each `name`.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicyQosGetResponse:
        """Create an instance of PolicyQosGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicyQosGetResponse:
        """Create an instance of PolicyQosGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicyQosGetResponse.parse_obj(obj)

        _obj = PolicyQosGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PolicyQos.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
