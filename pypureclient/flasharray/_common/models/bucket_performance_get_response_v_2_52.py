...
from pypureclient.flasharray._common.models.bucket_performance_v_2_52 import BucketPerformance
...
class BucketPerformanceGetResponse(BaseModel):
    ...
    items: Optional[conlist(BucketPerformance)] = Field(default=None, description="Performance data. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(BucketPerformance)] = Field(default=None, description="The aggregate value of all items after filtering. When it makes sense, the average value is displayed instead. The values are displayed for each field if meaningful.")
