...
from pypureclient.flasharray._common.models.policy_tls_v_2_54 import PolicyTls


class PolicyTlsGetResponse(BaseModel):
    """
    PolicyTlsGetResponse
    """
    ...
    items: Optional[conlist(PolicyTls)] = Field(default=None, description="The operation that returns a list of all items after filtering. If applicable, the values are displayed for each `name`.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicyTlsGetResponse:
        """Create an instance of PolicyTlsGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicyTlsGetResponse:
        """Create an instance of PolicyTlsGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicyTlsGetResponse.parse_obj(obj)

        _obj = PolicyTlsGetResponse.construct(_fields_set=None, **{
            ...
            "items": [PolicyTls.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
