...
from pypureclient.flasharray._common.models.realm_remote_array_status_v_2_55 import RealmRemoteArrayStatus
from pypureclient.flasharray._common.models.reference_with_type_v_2_54 import ReferenceWithType


class RemoteRealm(BaseModel):
    """
    RemoteRealm
    """
    ...
    arrays: Optional[conlist(RealmRemoteArrayStatus, min_items=1)] = None
    members: Optional[conlist(ReferenceWithType, min_items=1)] = Field(default=None, description="The list of `arrays` over which the `realm` is stretched. If there are two or more `members` in the `stretched realm`, all `data` in the `realm` is synchronously replicated among all of the `members`.")
    __properties = ["context", "id", "name", "arrays", "members"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RemoteRealm:
        """Create an instance of RemoteRealm from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RemoteRealm:
        """Create an instance of RemoteRealm from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RemoteRealm.parse_obj(obj)

        _obj = RemoteRealm.construct(_fields_set=None, **{
            "context": FixedReferenceWithType.from_dict(obj.get("context")) if obj.get("context") is not None else None,
            "id": obj.get("id"),
            "name": obj.get("name"),
            "arrays": [RealmRemoteArrayStatus.from_dict(_item) for _item in obj.get("arrays")] if obj.get("arrays") is not None else None,
