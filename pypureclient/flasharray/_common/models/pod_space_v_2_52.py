...
class PodSpace(BaseModel):
    ...
    footprint: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The maximum amount of physical space the container consumes on an array, ignoring any data shared outside the container, measured in bytes. On Evergreen//One arrays, this is the maximum amount of effective used space. The footprint metric is mostly used for capacity planning. This field will be null in non-container contexts.")
    ...
    total_physical: Optional[conint(strict=True, ge=0)] = Field(default=None, description="This field has been deprecated. Use the `total_used` field, as it contains the same information.")
    total_provisioned: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The provisioned size of a volume for a single volume, host or host group, protocol endpoint, managed directory, and containers can be infinite or measured in bytes. Infinite is represented by `null`. The provisioned size for a host or host group, includes all volumes that are connected to the resource. The provisioned size for a protocol endpoint is `null`. The provisioned size for a managed directory is the quota limit if it or its parent has a managed directory configured, otherwise it defaults to `null`. The provisioned size for a container is the sum of the total_provisioned of the object it contains, capped by the container's quota limit (or the container's used_provisioned if current usage is above the quota limit), if any. Provisioned size represents the storage capacity reported to hosts.")
    ...
    total_used: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The total space contributed by customer data, measured in bytes.")
    ...
    used_provisioned: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The amount of logical space a container has consumed, compared against the quota limit if the container has one configured. Used provisioned does not include destroyed objects inside the container. Used provisioned can include destroyed objects for a destroyed container and represents how much logical space it would take to recover the container.")
    ...
    replication: Optional[conint(strict=True, ge=0)] = Field(default=None, description="The space consumed by unique data that has not been replicated to the target pod, on source pods. The space consumed by unique data that has been replicated to the target, but not yet incorporated into the target pod, on target pods. On Evergreen//One arrays, the effective space consumed by unique data that has not been replicated to the target pod, on source pods. The effective space consumed by unique data that has been replicated to the target, but not yet incorporated into the target pod, on target pods.")
    replication_effective: Optional[conint(strict=True, ge=0)] = Field(default=None, description="This field has been deprecated. The effective space consumed by unique data that has not been replicated to the target pod, on source pods. The effective space consumed by unique data that has been replicated to the target but not yet incorporated into the target pod, on target pods. Please use the `replication` field in the future, as it contains the same information for Evergreen//One arrays.")
    shared_effective: Optional[conint(strict=True, ge=0)] = Field(default=None, description="This field has been deprecated. The effective space contributed by data that is not unique to a specific volume, managed directory, or snapshot, measured in bytes. Please use the `shared` field in the future, as it contains the same information for Evergreen//One arrays.")
    __properties = ["data_reduction", "footprint", "shared", "snapshots", "system", "thin_provisioning", "total_physical", "total_provisioned", "total_reduction", "total_used", "unique", "used_provisioned", "virtual", "replication", "replication_effective", "shared_effective"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PodSpace:
        """Create an instance of PodSpace from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PodSpace:
        """Create an instance of PodSpace from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PodSpace.parse_obj(obj)

        _obj = PodSpace.construct(_fields_set=None, **{
