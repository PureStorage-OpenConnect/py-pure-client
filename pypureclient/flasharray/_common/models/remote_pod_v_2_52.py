...
from pypureclient.flasharray._common.models.pod_remote_array_status_v_2_52 import PodRemoteArrayStatus
from pypureclient.flasharray._common.models.reference_with_type_v_2_52 import ReferenceWithType


class RemotePod(BaseModel):
    """
    RemotePod
    """
    ...
    arrays: Optional[conlist(PodRemoteArrayStatus, min_items=1)] = None
    members: Optional[conlist(ReferenceWithType, min_items=1)] = Field(default=None, description="The list of arrays or realms over which the pod is stretched. If there are two or more members in the stretched pod, all data in the pod is synchronously replicated between all members within the pod.")
    __properties = ["id", "name", "context", "arrays", "members"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RemotePod:
        """Create an instance of RemotePod from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RemotePod:
        """Create an instance of RemotePod from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RemotePod.parse_obj(obj)

        _obj = RemotePod.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
            "context": FixedReferenceWithType.from_dict(obj.get("context")) if obj.get("context") is not None else None,
            "arrays": [PodRemoteArrayStatus.from_dict(_item) for _item in obj.get("arrays")] if obj.get("arrays") is not None else None,
