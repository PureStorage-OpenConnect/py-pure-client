...
from pypureclient.flasharray._common.models.resource_bucket_space_v_2_52 import ResourceBucketSpace


class ResourceBucketSpaceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourceBucketSpace)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourceBucketSpace)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourceBucketSpaceGetResponse:
        """Create an instance of ResourceBucketSpaceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourceBucketSpaceGetResponse:
        """Create an instance of ResourceBucketSpaceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourceBucketSpaceGetResponse.parse_obj(obj)

        _obj = ResourceBucketSpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourceBucketSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourceBucketSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
