...
from pypureclient.flasharray._common.models.fleet_member_v_2_44 import FleetMember


class FleetMemberGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(FleetMember)] = Field(default=None, description="Returns a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FleetMemberGetResponse:
        """Create an instance of FleetMemberGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FleetMemberGetResponse:
        """Create an instance of FleetMemberGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FleetMemberGetResponse.parse_obj(obj)

        _obj = FleetMemberGetResponse.construct(_fields_set=None, **{
            ...
            "items": [FleetMember.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
