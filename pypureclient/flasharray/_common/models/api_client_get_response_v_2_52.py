...
from pypureclient.flasharray._common.models.api_client_v_2_52 import ApiClient


class ApiClientGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ApiClient)] = Field(default=None, description="Returns a list of all items after filtering. The values are displayed for each name, where meaningful.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ApiClientGetResponse:
        """Create an instance of ApiClientGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ApiClientGetResponse:
        """Create an instance of ApiClientGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ApiClientGetResponse.parse_obj(obj)

        _obj = ApiClientGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ApiClient.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
