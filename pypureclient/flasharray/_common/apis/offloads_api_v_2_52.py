...
from pypureclient.flasharray._common.models.offload_get_response_v_2_52 import OffloadGetResponse
from pypureclient.flasharray._common.models.offload_patch_v_2_52 import OffloadPatch
from pypureclient.flasharray._common.models.offload_post_v_2_52 import OffloadPost
from pypureclient.flasharray._common.models.offload_response_v_2_52 import OffloadResponse
...
class OffloadsApi:
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    ...
                    " to method api252_offloads_delete" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
        ...
        **kwargs
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api252_offloads_get" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
    ...
    def offloads_patch_with_http_info(
        self,
        offload: OffloadPatch,
        authorization: Annotated[Optional[StrictStr], Field(description="Access token (in JWT format) required to use any API endpoint (except `/oauth2`, `/login`, and `/logout`)")] = None,
        x_request_id: Annotated[Optional[StrictStr], Field(description="Supplied by client during request or generated by server.")] = None,
        context_names: Annotated[Optional[conlist(StrictStr)], Field(description="Performs the operation on the context specified. If specified, the context names must be an array of size 1, and the single element must be the name of an array in the same fleet or the name of the fleet itself. If not specified, the context will default to the array that received this request. Other parameters provided with the request, such as names of volumes or snapshots, are resolved relative to the provided `context`.")] = None,
        names: Annotated[Optional[conlist(StrictStr)], Field(description="Performs the operation on the unique names specified. Enter multiple names in comma-separated format. For example, `name01,name02`. If there is not at least one resource that matches each of the elements of `names`, then an error is returned, except when creating new resources.")] = None,
        **kwargs
        ...
        """Patch offload target

        Modifies an existing offload target. Usage includes rotating the client secret.

        :param offload: (required)
        :type offload: OffloadPatch
        :param authorization: Access token (in JWT format) required to use any API endpoint (except `/oauth2`, `/login`, and `/logout`)
        :type authorization: str
        :param x_request_id: Supplied by client during request or generated by server.
        :type x_request_id: str
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api252_offloads_patch" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
        ...
        return self.api_client.call_api(
            f'/api/{self.version}/offloads', 'PATCH',
            _path_params,
            _query_params,
            _header_params,
            body=_body_params,
        ...
        **kwargs
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api252_offloads_post" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
