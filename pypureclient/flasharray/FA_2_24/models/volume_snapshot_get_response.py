...
class VolumeSnapshotGetResponse(BaseModel):
    ...
    items: Optional[List[VolumeSnapshot]] = Field(default=None, description="Returns a list of all items after filtering. The values are displayed for each name, if appliclable. If `total_only=true`, the `items` list will be empty.")
    total: Optional[List[VolumeSnapshot]] = Field(default=None, description="The aggregate value of all items after filtering. If applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
