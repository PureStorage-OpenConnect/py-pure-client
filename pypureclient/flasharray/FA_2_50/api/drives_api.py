...
from pypureclient.flasharray.common.models.drive_get_response_v_42 import DriveGetResponse
from pypureclient.flasharray.common.models.drive_response_v_42 import DriveResponse
...
class DrivesApi:
    ...
    def api250_drives_get_with_http_info(
        self,
        ...
        **kwargs
        ...
        :rtype: tuple(DriveGetResponse, status_code(int), headers(HTTPHeaderDict))
        """
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    ...
                    " to method api250_drives_get" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
    ...
    def api250_drives_patch_with_http_info(
        self,
        authorization: Annotated[Optional[StrictStr], Field(description="Access token (in JWT format) required to use any API endpoint (except `/oauth2`, `/login`, and `/logout`)")] = None,
        x_request_id: Annotated[Optional[StrictStr], Field(description="Supplied by client during request or generated by server.")] = None,
        ...
        **kwargs
        ...
        :return: Returns the result object.
                 If the method is called asynchronously,
                 returns the request thread.
        :rtype: tuple(DriveResponse, status_code(int), headers(HTTPHeaderDict))
        """
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api250_drives_patch" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
        ...
        # process the query parameters
        _query_params = []
        if _params.get('admit') is not None:  # noqa: E501
            _query_params.append(('admit', _params['admit']))
