...
class TargetProtectionGroup(BaseModel):
    ...
    status: Optional[StrictStr] = Field(default=None, description="The replication status of the target. Valid values are `replicating`, `suspended`, and `disallowed`. If `allowed` is `true` and protection group data is replicating to the target, `status` will display `replicating`. If `allowed` is `true`, but replication is suspended due to the target being demoted, linked, stretched or there is a missing realm connection to the target; `status` will display `suspended`. Replication will resume automatically once the  conditions for suspend no longer hold. If `allowed` is `false`, `status` will display `disallowed`.")
