...
from pypureclient.flasharray.FA_2_50.models.networkinterfacepost_eth import NetworkinterfacepostEth
...
class NetworkInterfacePost(BaseModel):
    ...
    attached_servers: Optional[conlist(Reference, max_items=1)] = Field(default=None, description="Applies only to Ethernet interfaces. The list of servers to be associated with the specified network interface for data ingress. At most one server can be specified for each interface. To attach the network interface to a server, the name or id of the desired server must be provided. If not specified and the interface is created at the array level, defaults to the _array_server instance. If not specified and the interface is created within a server, defaults to that server. To create a network interface that is not attached to any server, specify an empty list [].")
