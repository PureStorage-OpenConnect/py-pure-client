...
from pypureclient.flasharray.FA_2_50.models.policy_lockable_eradication_config import PolicyLockableEradicationConfig
...
class PolicyLockable(BaseModel):
    ...
    eradication_config: Optional[PolicyLockableEradicationConfig] = None
    retention_lock: Optional[StrictStr] = Field(default=None, description="The valid values are `ratcheted` and `unlocked`. The default value for a newly created policy is `unlocked`. Set `retention_lock` to `ratcheted` to enable SafeMode restrictions on the policy. Contact Pure Technical Services to change `retention_lock` to `unlocked`.")
    __properties = ["id", "name", "context", "destroyed", "enabled", "pod", "policy_type", "time_remaining", "eradication_config", "retention_lock"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PolicyLockable:
        """Create an instance of PolicyLockable from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PolicyLockable:
        """Create an instance of PolicyLockable from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PolicyLockable.parse_obj(obj)

        _obj = PolicyLockable.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
            ...
            "eradication_config": PolicyLockableEradicationConfig.from_dict(obj.get("eradication_config")) if obj.get("eradication_config") is not None else None,
