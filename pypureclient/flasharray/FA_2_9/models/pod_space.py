...
class PodSpace(BaseModel):
    ...
    replication: Optional[conint(strict=True, ge=0)] = Field(default=None, description="On source pods, the space consumed by unique data that has not been replicated to the target pod. On target pods, the space consumed by unique data that has been replicated to the target but not yet incorporated into the target pod. On Evergreen//One arrays, this is the effective space consumed by unique data that has not been replicated to the target pod, on source pods. The effective space consumed by unique data that has been replicated to the target but not yet incorporated into the target pod, on target pods.")
    __properties = ["data_reduction", "shared", "snapshots", "system", "thin_provisioning", "total_physical", "total_provisioned", "total_reduction", "unique", "virtual", "replication"]
