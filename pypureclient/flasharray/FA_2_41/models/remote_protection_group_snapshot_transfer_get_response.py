...
from pypureclient.flasharray.FA_2_41.models.remote_protection_group_snapshot_transfer import RemoteProtectionGroupSnapshotTransfer


class RemoteProtectionGroupSnapshotTransferGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(RemoteProtectionGroupSnapshotTransfer)] = None
    ...
    total: Optional[conlist(RemoteProtectionGroupSnapshotTransfer)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RemoteProtectionGroupSnapshotTransferGetResponse:
        """Create an instance of RemoteProtectionGroupSnapshotTransferGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RemoteProtectionGroupSnapshotTransferGetResponse:
        """Create an instance of RemoteProtectionGroupSnapshotTransferGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RemoteProtectionGroupSnapshotTransferGetResponse.parse_obj(obj)

        _obj = RemoteProtectionGroupSnapshotTransferGetResponse.construct(_fields_set=None, **{
            ...
            "items": [RemoteProtectionGroupSnapshotTransfer.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [RemoteProtectionGroupSnapshotTransfer.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
