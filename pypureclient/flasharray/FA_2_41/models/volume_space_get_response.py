...
from pypureclient.flasharray.FA_2_41.models.volume_space import VolumeSpace


class VolumeSpaceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(VolumeSpace)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(VolumeSpace)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> VolumeSpaceGetResponse:
        """Create an instance of VolumeSpaceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> VolumeSpaceGetResponse:
        """Create an instance of VolumeSpaceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return VolumeSpaceGetResponse.parse_obj(obj)

        _obj = VolumeSpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [VolumeSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [VolumeSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
