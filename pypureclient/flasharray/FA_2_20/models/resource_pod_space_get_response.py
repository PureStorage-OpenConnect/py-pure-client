...
from pypureclient.flasharray.FA_2_20.models.resource_pod_space import ResourcePodSpace
...
class ResourcePodSpaceGetResponse(BaseModel):
    ...
    items: Optional[conlist(ResourcePodSpace)] = Field(default=None, description="Displays a list of all items after filtering. The values are displayed for each name where meaningful. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePodSpace)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. The values are displayed for each field if meaningful.")
