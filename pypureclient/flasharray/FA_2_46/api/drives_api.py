...
from pypureclient.flasharray.FA_2_46.models.drive_get_response import DriveGetResponse
from pypureclient.flasharray.FA_2_46.models.drive_response import DriveResponse
from typing import Optional
...
class DrivesApi:
    ...
    def api246_drives_get_with_http_info(
        self,
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    ...
                    " to method api246_drives_get" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
    ...
    def api246_drives_patch_with_http_info(
        self,
        authorization: Annotated[Optional[StrictStr], Field(description="Access token (in JWT format) required to use any API endpoint (except `/oauth2`, `/login`, and `/logout`)")] = None,
        x_request_id: Annotated[Optional[StrictStr], Field(description="Supplied by client during request or generated by server.")] = None,
        ...
        **kwargs
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api246_drives_patch" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
