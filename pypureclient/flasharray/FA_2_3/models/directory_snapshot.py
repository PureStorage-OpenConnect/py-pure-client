...
class DirectorySnapshot(object):
    ...
    def __init__(
        ...
        Keyword args:
            ...
            client_name (str): The customizable portion of the client visible snapshot name. A full snapshot name is constructed in the form of `DIR.CLIENT_NAME.NNN` where `DIR` is the full managed directory name, `CLIENT_NAME` is the client name, and `NNN` is a monotonically increasing number generated by the system. The client visible snapshot name is `CLIENT_NAME.NNN`.
            ...
            suffix (int): The suffix that is appended to the `source_name` value to generate the full directory snapshot name in the form of `DIR.CLIENT_NAME.NNN` where `DIR` is the managed directory name and `CLIENT_NAME.NNN` is the suffix. The suffix is constructed from a customizable client name `CLIENT_NAME` and a monotonically increasing number generated by the system `NNN`. The suffix is the client visible snapshot name.
