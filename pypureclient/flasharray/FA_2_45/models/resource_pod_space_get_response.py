...
from pypureclient.flasharray.FA_2_45.models.resource_pod_space import ResourcePodSpace


class ResourcePodSpaceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePodSpace)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name. If `total_only=true`, the `items` list will be empty.")
    total: Optional[conlist(ResourcePodSpace)] = Field(default=None, description="The aggregate value of all items after filtering. When applicable, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePodSpaceGetResponse:
        """Create an instance of ResourcePodSpaceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePodSpaceGetResponse:
        """Create an instance of ResourcePodSpaceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePodSpaceGetResponse.parse_obj(obj)

        _obj = ResourcePodSpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ResourcePodSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePodSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
