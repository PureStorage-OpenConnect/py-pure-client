...
from pypureclient.flasharray.FA_2_38.models.protection_group_snapshot_transfer import ProtectionGroupSnapshotTransfer


class ProtectionGroupSnapshotTransferGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ProtectionGroupSnapshotTransfer)] = Field(default=None, description="Displays a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    total: Optional[conlist(ProtectionGroupSnapshotTransfer)] = Field(default=None, description="The aggregate value of all items after filtering. Where it makes more sense, the average value is displayed instead. If applicable, the values are displayed for each field.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProtectionGroupSnapshotTransferGetResponse:
        """Create an instance of ProtectionGroupSnapshotTransferGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ProtectionGroupSnapshotTransferGetResponse:
        """Create an instance of ProtectionGroupSnapshotTransferGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProtectionGroupSnapshotTransferGetResponse.parse_obj(obj)

        _obj = ProtectionGroupSnapshotTransferGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ProtectionGroupSnapshotTransfer.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorcontextresponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": [ProtectionGroupSnapshotTransfer.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
