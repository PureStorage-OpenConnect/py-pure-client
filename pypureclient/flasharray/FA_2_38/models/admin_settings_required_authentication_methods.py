...
class AdminSettingsRequiredAuthenticationMethods(BaseModel):
    ...
    """ # noqa: E501
    ssh: Optional[List[StrictStr]] = Field(default=None, description="List of authentication methods that are required for SSH. Possible values include `password`, `key`, and `default`. `securid-am` is a possible value, but it cannot be changed through PATCH. If not specified, defaults to `default`. Specify `password` and `key` authentication methods to set up local multi-factor authentication for SSH.")
    web_ui: Optional[List[StrictStr]] = Field(default=None, description="List of authentication methods that are required for the web UI. Possible values include `password`, `webauthn`, and `default`. Other possible values include `saml2` and `securid-am` but these cannot be changed through PATCH. If not specified, defaults to `default`. Specify `password` and `webauthn` authentication methods to set up local multi-factor authentication for web UI. External multi-factor authentication is configured through SAML2 SSO endpoints.")
    __properties: ClassVar[List[str]] = ["ssh", "web_ui"]
