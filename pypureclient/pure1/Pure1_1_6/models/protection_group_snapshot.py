...
class ProtectionGroupSnapshot(BaseModel):
    """
    ...
    on: Optional[FixedReferenceFqdn] = Field(default=None, description="A reference to the array where the snapshot is stored.")
    pod: Optional[FixedReference] = Field(default=None, description="A reference to the pod the source protection group belongs to, if applicable.")
    source: Optional[FixedReference] = Field(default=None, description="A reference to the protection group that the snapshot was taken from.")
    __properties = ["_as_of", "id", "name", "created", "on", "pod", "source"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ProtectionGroupSnapshot:
        """Create an instance of ProtectionGroupSnapshot from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ProtectionGroupSnapshot:
        """Create an instance of ProtectionGroupSnapshot from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ProtectionGroupSnapshot.parse_obj(obj)

        _obj = ProtectionGroupSnapshot.construct(_fields_set=None, **{
