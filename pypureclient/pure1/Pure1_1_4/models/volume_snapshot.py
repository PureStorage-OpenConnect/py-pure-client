...
class VolumeSnapshot(BaseModel):
    """
    A point-in-time image of the contents of a volume.  # noqa: E501
    """
    ...
    on: Optional[FixedReferenceFqdn] = Field(default=None, description="A reference to the array or the offload where the snapshot is stored.")
    pod: Optional[FixedReference] = Field(default=None, description="A reference to the pod the source volume belongs to, if applicable.")
    provisioned: Optional[StrictInt] = Field(default=None, description="Indicates the size (in bytes) of the volume when the snapshot was taken.")
    serial: Optional[StrictStr] = Field(default=None, description="Serial number generated by Purity when the snapshot was created.")
    snapshot_group: Optional[FixedReference] = Field(default=None, description="A reference to a consistency group snapshot that this snapshot is part of.")
    source: Optional[FixedReference] = Field(default=None, description="A reference to the volume that the snapshot was taken from.")
    suffix: Optional[StrictStr] = Field(default=None, description="Suffix added to the source volume name used to generate the volume snapshot name.")
    __properties = ["_as_of", "id", "name", "arrays", "created", "destroyed", "on", "pod", "provisioned", "serial", "snapshot_group", "source", "suffix"]
    ...
    def to_dict(self, include_readonly: bool=True) -> Dict[str, Any]:
        ...
        if _include_in_dict('on', include_readonly, excluded_fields, none_fields):
            _dict['on'] = self.on.to_dict(include_readonly=include_readonly)
        ...
        # override the default output from pydantic by calling `to_dict()` of snapshot_group
        if _include_in_dict('snapshot_group', include_readonly, excluded_fields, none_fields):
            _dict['snapshot_group'] = self.snapshot_group.to_dict(include_readonly=include_readonly)
    ...
    def from_dict(cls, obj: dict) -> VolumeSnapshot:
        ...
        _obj = VolumeSnapshot.construct(_fields_set=None, **{
            ...
            "on": FixedReferenceFqdn.from_dict(obj.get("on")) if obj.get("on") is not None else None,
            "pod": FixedReference.from_dict(obj.get("pod")) if obj.get("pod") is not None else None,
            "provisioned": obj.get("provisioned"),
            "serial": obj.get("serial"),
            "snapshot_group": FixedReference.from_dict(obj.get("snapshot_group")) if obj.get("snapshot_group") is not None else None,
