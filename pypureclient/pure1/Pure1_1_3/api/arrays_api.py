...
class ArraysApi(object):
    ...
    def api13_arrays_tags_batch_put_with_http_info(
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api13_arrays_tags_batch_put_with_http_info(error_unknown, async_req=True)
        >>> result = thread.get()
        ...
        if error_unknown is None:
            raise TypeError("Missing the required parameter `error_unknown` when calling `api13_arrays_tags_batch_put`")
