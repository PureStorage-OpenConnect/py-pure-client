...
class BladesApi(object):
    ...
    def api23_blades_get_with_http_info(
        ...
        >>> thread = api.api23_blades_get_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        if 'limit' in params and params['limit'] < 1:
            raise ValueError("Invalid value for parameter `limit` when calling `api23_blades_get`, must be a value greater than or equal to `1`")
        if 'offset' in params and params['offset'] < 0:
            raise ValueError("Invalid value for parameter `offset` when calling `api23_blades_get`, must be a value greater than or equal to `0`")
        collection_formats = {}
        path_params = {}
