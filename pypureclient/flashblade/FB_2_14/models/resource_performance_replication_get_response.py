...
from pypureclient.flashblade.FB_2_14.models.resource_performance_replication import ResourcePerformanceReplication


class ResourcePerformanceReplicationGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ResourcePerformanceReplication)] = None
    total: Optional[conlist(ResourcePerformanceReplication)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ResourcePerformanceReplicationGetResponse:
        """Create an instance of ResourcePerformanceReplicationGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ResourcePerformanceReplicationGetResponse:
        """Create an instance of ResourcePerformanceReplicationGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ResourcePerformanceReplicationGetResponse.parse_obj(obj)

        _obj = ResourcePerformanceReplicationGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [ResourcePerformanceReplication.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ResourcePerformanceReplication.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
