...
from pypureclient.flashblade.FB_2_12.models.policy_base_get_response import PolicyBaseGetResponse
...
class PoliciesAllApi:
    ...
    def api212_policies_all_get_with_http_info(
        self,
        ...
        **kwargs
        ...
        >>> thread = api.api212_policies_all_get_with_http_info(continuation_token, filter, ids, limit, names, offset, sort, async_req=True)
        >>> result = thread.get()
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    ...
                    " to method api212_policies_all_get" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
    ...
    def api212_policies_all_members_get_with_http_info(
        self,
        continuation_token: Annotated[Optional[StrictStr], Field(description="An opaque token used to iterate over a collection. The token to use on the next request is returned in the `continuation_token` field of the result.")] = None,
        filter: Annotated[Optional[StrictStr], Field(description="Exclude resources that don't match the specified criteria.")] = None,
        limit: Annotated[Optional[conint(strict=True, ge=0)], Field(description="Limit the size of the response to the specified number of resources. A `limit` of `0` can be used to get the number of resources without getting all of the resources. It will be returned in the `total_item_count` field. If a client asks for a page size larger than the available number, the request is still valid. In that case the server just returns the available number of items, disregarding the client's page size request.")] = None,
        ...
        **kwargs
        ...
        >>> thread = api.api212_policies_all_members_get_with_http_info(continuation_token, filter, limit, local_file_system_ids, local_file_system_names, member_ids, member_names, member_types, offset, policy_ids, policy_names, remote_file_system_ids, remote_file_system_names, remote_ids, remote_names, sort, async_req=True)
        >>> result = thread.get()
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api212_policies_all_members_get" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
        ...
        return self.api_client.call_api(
            '/api/2.12/policies-all/members', 'GET',
            _path_params,
            _query_params,
            _header_params,
            body=_body_params,
