...
from pypureclient.flashblade.FB_2_19.models.drive import Drive


class DriveGetResponse(BaseModel):
    """
    DriveGetResponse
    """
    ...
    items: Optional[conlist(Drive)] = Field(default=None, description="A list of drive objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DriveGetResponse:
        """Create an instance of DriveGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DriveGetResponse:
        """Create an instance of DriveGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DriveGetResponse.parse_obj(obj)

        _obj = DriveGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [Drive.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": obj.get("total")
        })
        return _obj
