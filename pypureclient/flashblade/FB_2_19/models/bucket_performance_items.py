...
class BucketPerformanceItems(BaseModel):
    """
    ...
    max_total_bytes_per_sec: Optional[Union[confloat(ge=0, strict=True), conint(ge=0, strict=True)]] = Field(default=None, description="Limit on throughput in bytes per bucket.")
    max_total_ops_per_sec: Optional[Union[confloat(ge=0, strict=True), conint(ge=0, strict=True)]] = Field(default=None, description="Limit on operation count per bucket.")
    ...
    __properties = ["id", "name", "bytes_per_op", "bytes_per_read", "bytes_per_write", "max_total_bytes_per_sec", "max_total_ops_per_sec", "others_per_sec", "read_bytes_per_sec", "reads_per_sec", "time", "usec_per_other_op", "usec_per_read_op", "usec_per_write_op", "write_bytes_per_sec", "writes_per_sec"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BucketPerformanceItems:
        """Create an instance of BucketPerformanceItems from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> BucketPerformanceItems:
        """Create an instance of BucketPerformanceItems from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BucketPerformanceItems.parse_obj(obj)

        _obj = BucketPerformanceItems.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
