...
from pypureclient.flashblade.FB_2_17.models.client_performance import ClientPerformance
from pypureclient.flashblade.FB_2_17.models.error_context_response_errors import ErrorContextResponseErrors


class ClientPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ClientPerformance)] = Field(default=None, description="A list of client performance metrics objects.")
    total: Optional[conlist(ClientPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClientPerformanceGetResponse:
        """Create an instance of ClientPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ClientPerformanceGetResponse:
        """Create an instance of ClientPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClientPerformanceGetResponse.parse_obj(obj)

        _obj = ClientPerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ClientPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ClientPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
