...
class AlertsApi(object):
    ...
    def api24_alerts_patch_with_http_info(
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api24_alerts_patch_with_http_info(alerts_settings, async_req=True)
        >>> result = thread.get()
        ...
        if alerts_settings is None:
            raise TypeError("Missing the required parameter `alerts_settings` when calling `api24_alerts_patch`")
