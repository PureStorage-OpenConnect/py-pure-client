...
from pypureclient.flashblade._common.models.network_interface_trace_v_2_26 import NetworkInterfaceTrace


class NetworkInterfaceTraceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(NetworkInterfaceTrace)] = Field(default=None, description="A list of network trace run result.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NetworkInterfaceTraceGetResponse:
        """Create an instance of NetworkInterfaceTraceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NetworkInterfaceTraceGetResponse:
        """Create an instance of NetworkInterfaceTraceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NetworkInterfaceTraceGetResponse.parse_obj(obj)

        _obj = NetworkInterfaceTraceGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [NetworkInterfaceTrace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
