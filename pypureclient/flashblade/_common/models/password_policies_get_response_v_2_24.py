...
from pypureclient.flashblade._common.models.password_policy_v_2_24 import PasswordPolicy


class PasswordPoliciesGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PasswordPolicy)] = Field(default=None, description="Returns a list of all items after filtering. If applicable, the values are displayed for each name.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PasswordPoliciesGetResponse:
        """Create an instance of PasswordPoliciesGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PasswordPoliciesGetResponse:
        """Create an instance of PasswordPoliciesGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PasswordPoliciesGetResponse.parse_obj(obj)

        _obj = PasswordPoliciesGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [PasswordPolicy.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
