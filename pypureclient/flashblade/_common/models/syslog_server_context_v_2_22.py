...
class SyslogServerContext(BaseModel):
    ...
    sources: Optional[conlist(Reference, max_items=1)] = Field(default=None, description="The network interfaces used for communication with the syslog server. The network interfaces must have the `services` value of `data`. Only network interfaces within default array realm are supported.")
    ...
    __properties = ["uri", "services", "sources", "id", "name", "context"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SyslogServerContext:
        """Create an instance of SyslogServerContext from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> SyslogServerContext:
        """Create an instance of SyslogServerContext from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SyslogServerContext.parse_obj(obj)

        _obj = SyslogServerContext.construct(_fields_set=None, **{
            "uri": obj.get("uri"),
