...
class AdminApiToken(BaseModel):
    ...
    admin: Optional[Reference] = Field(default=None, description="A reference to the admin to whom this token belongs.")
    api_token: Optional[ApiToken] = None
    __properties = ["context", "admin", "api_token"]
    ...
    def to_dict(self, include_readonly: bool=True) -> Dict[str, Any]:
        ...
        if _include_in_dict('admin', include_readonly, excluded_fields, none_fields):
            _dict['admin'] = self.admin.to_dict(include_readonly=include_readonly)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AdminApiToken:
        """Create an instance of AdminApiToken from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> AdminApiToken:
        """Create an instance of AdminApiToken from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AdminApiToken.parse_obj(obj)

        _obj = AdminApiToken.construct(_fields_set=None, **{
            "context": FixedReference.from_dict(obj.get("context")) if obj.get("context") is not None else None,
            "admin": Reference.from_dict(obj.get("admin")) if obj.get("admin") is not None else None,
