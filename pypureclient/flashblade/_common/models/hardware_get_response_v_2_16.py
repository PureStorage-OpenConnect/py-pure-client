...
from pypureclient.flashblade._common.models.hardware_v_2_16 import Hardware


class HardwareGetResponse(BaseModel):
    """
    HardwareGetResponse
    """
    ...
    items: Optional[conlist(Hardware)] = Field(default=None, description="A list of hardware component objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> HardwareGetResponse:
        """Create an instance of HardwareGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> HardwareGetResponse:
        """Create an instance of HardwareGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HardwareGetResponse.parse_obj(obj)

        _obj = HardwareGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [Hardware.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
