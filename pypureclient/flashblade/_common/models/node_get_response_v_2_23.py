...
from pypureclient.flashblade._common.models.node_v_2_23 import Node
...
class NodeGetResponse(BaseModel):
    ...
    items: Optional[conlist(Node)] = Field(default=None, description="A list of node objects.")
    total: Optional[Node] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NodeGetResponse:
        """Create an instance of NodeGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NodeGetResponse:
        """Create an instance of NodeGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NodeGetResponse.parse_obj(obj)

        _obj = NodeGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [Node.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": Node.from_dict(obj.get("total")) if obj.get("total") is not None else None
        })
        return _obj
