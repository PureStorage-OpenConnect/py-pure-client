...
from pypureclient.flashblade._common.models.clients_performance_v_2_24 import ClientsPerformance
from pypureclient.flashblade._common.models.error_context_response_errors_v_2_24 import ErrorContextResponseErrors


class ClientsPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ClientsPerformance)] = Field(default=None, description="A list of client performance metrics objects.")
    total: Optional[conlist(ClientsPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    __properties = ["continuation_token", "total_item_count", "errors", "items", "total"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ClientsPerformanceGetResponse:
        """Create an instance of ClientsPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ClientsPerformanceGetResponse:
        """Create an instance of ClientsPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ClientsPerformanceGetResponse.parse_obj(obj)

        _obj = ClientsPerformanceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ClientsPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ClientsPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
