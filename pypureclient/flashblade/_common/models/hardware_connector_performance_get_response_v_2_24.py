...
from pypureclient.flashblade._common.models.hardware_connector_performance_v_2_24 import HardwareConnectorPerformance


class HardwareConnectorPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(HardwareConnectorPerformance)] = Field(default=None, description="A list of hardware connector performance objects.")
    total: Optional[conlist(HardwareConnectorPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no individual items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> HardwareConnectorPerformanceGetResponse:
        """Create an instance of HardwareConnectorPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> HardwareConnectorPerformanceGetResponse:
        """Create an instance of HardwareConnectorPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return HardwareConnectorPerformanceGetResponse.parse_obj(obj)

        _obj = HardwareConnectorPerformanceGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [HardwareConnectorPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [HardwareConnectorPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
