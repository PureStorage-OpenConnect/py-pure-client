...
from pypureclient.flashblade._common.models.tls_policy_v_2_24 import TlsPolicy


class TlsPolicyGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(TlsPolicy)] = Field(default=None, description="A list of TLS policy objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> TlsPolicyGetResponse:
        """Create an instance of TlsPolicyGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> TlsPolicyGetResponse:
        """Create an instance of TlsPolicyGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return TlsPolicyGetResponse.parse_obj(obj)

        _obj = TlsPolicyGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [TlsPolicy.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
