...
from pypureclient.flashblade._common.models.network_interface_ping_v_2_26 import NetworkInterfacePing


class NetworkInterfacePingGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(NetworkInterfacePing)] = Field(default=None, description="A list of network ping run result.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NetworkInterfacePingGetResponse:
        """Create an instance of NetworkInterfacePingGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NetworkInterfacePingGetResponse:
        """Create an instance of NetworkInterfacePingGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NetworkInterfacePingGetResponse.parse_obj(obj)

        _obj = NetworkInterfacePingGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [NetworkInterfacePing.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
