...
from pypureclient.flashblade._common.models.object_store_account_v_2_24 import ObjectStoreAccount


class ObjectStoreAccountGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(ObjectStoreAccount)] = Field(default=None, description="A list of object store account objects.")
    total: Optional[ObjectStoreAccount] = Field(default=None, description="If `total_only` query param is `true`, then this field will be a total of all records after filtering and no items will be returned. If `total_only` is `false`, then it will be a total of all records on this page.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ObjectStoreAccountGetResponse:
        """Create an instance of ObjectStoreAccountGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ObjectStoreAccountGetResponse:
        """Create an instance of ObjectStoreAccountGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ObjectStoreAccountGetResponse.parse_obj(obj)

        _obj = ObjectStoreAccountGetResponse.construct(_fields_set=None, **{
            ...
            "items": [ObjectStoreAccount.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": ObjectStoreAccount.from_dict(obj.get("total")) if obj.get("total") is not None else None
        })
        return _obj
