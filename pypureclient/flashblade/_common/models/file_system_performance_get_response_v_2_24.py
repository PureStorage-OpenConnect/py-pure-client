...
from pypureclient.flashblade._common.models.file_system_performance_v_2_24 import FileSystemPerformance


class FileSystemPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(FileSystemPerformance)] = Field(default=None, description="A list of file system performance objects.")
    total: Optional[conlist(FileSystemPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FileSystemPerformanceGetResponse:
        """Create an instance of FileSystemPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FileSystemPerformanceGetResponse:
        """Create an instance of FileSystemPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FileSystemPerformanceGetResponse.parse_obj(obj)

        _obj = FileSystemPerformanceGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [FileSystemPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [FileSystemPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
