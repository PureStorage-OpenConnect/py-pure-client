...
class AdminSettingsGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(AdminSetting)] = Field(default=None, description="Returns global admin settings.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AdminSettingsGetResponse:
        """Create an instance of AdminSettingsGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> AdminSettingsGetResponse:
        """Create an instance of AdminSettingsGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return AdminSettingsGetResponse.parse_obj(obj)

        _obj = AdminSettingsGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [AdminSetting.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
