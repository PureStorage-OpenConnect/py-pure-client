...
from pypureclient.flashblade._common.models.file_system_snapshot_v_2_17 import FileSystemSnapshot


class FileSystemSnapshotGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(FileSystemSnapshot)] = Field(default=None, description="A list of file system snapshot objects.")
    ...
    total: Optional[FileSystemSnapshot] = Field(default=None, description="If `total_only` query param is `true`, then it will be a total of all records after filtering and no items will be returned. If `total_only` is `false`, then it will be a total of all records on this page.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FileSystemSnapshotGetResponse:
        """Create an instance of FileSystemSnapshotGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FileSystemSnapshotGetResponse:
        """Create an instance of FileSystemSnapshotGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FileSystemSnapshotGetResponse.parse_obj(obj)

        _obj = FileSystemSnapshotGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [FileSystemSnapshot.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorContextResponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": FileSystemSnapshot.from_dict(obj.get("total")) if obj.get("total") is not None else None
        })
        return _obj
