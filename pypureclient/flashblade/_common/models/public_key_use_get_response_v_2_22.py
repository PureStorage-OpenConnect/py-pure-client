...
from pypureclient.flashblade._common.models.public_key_use_v_2_22 import PublicKeyUse


class PublicKeyUseGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PublicKeyUse)] = Field(default=None, description="A list of public key use objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PublicKeyUseGetResponse:
        """Create an instance of PublicKeyUseGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PublicKeyUseGetResponse:
        """Create an instance of PublicKeyUseGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PublicKeyUseGetResponse.parse_obj(obj)

        _obj = PublicKeyUseGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [PublicKeyUse.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
