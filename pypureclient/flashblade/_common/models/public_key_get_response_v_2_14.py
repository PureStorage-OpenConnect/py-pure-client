...
from pypureclient.flashblade._common.models.public_key_v_2_14 import PublicKey


class PublicKeyGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(PublicKey)] = Field(default=None, description="A list of public key objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PublicKeyGetResponse:
        """Create an instance of PublicKeyGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> PublicKeyGetResponse:
        """Create an instance of PublicKeyGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PublicKeyGetResponse.parse_obj(obj)

        _obj = PublicKeyGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [PublicKey.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
