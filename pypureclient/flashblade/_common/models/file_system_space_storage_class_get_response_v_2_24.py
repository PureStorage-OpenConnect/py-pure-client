...
from pypureclient.flashblade._common.models.file_system_storage_class_space_v_2_24 import FileSystemStorageClassSpace


class FileSystemSpaceStorageClassGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(FileSystemStorageClassSpace)] = Field(default=None, description="A list of file system storage class space objects.")
    total: Optional[conlist(FileSystemStorageClassSpace)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FileSystemSpaceStorageClassGetResponse:
        """Create an instance of FileSystemSpaceStorageClassGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FileSystemSpaceStorageClassGetResponse:
        """Create an instance of FileSystemSpaceStorageClassGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FileSystemSpaceStorageClassGetResponse.parse_obj(obj)

        _obj = FileSystemSpaceStorageClassGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [FileSystemStorageClassSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [FileSystemStorageClassSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
