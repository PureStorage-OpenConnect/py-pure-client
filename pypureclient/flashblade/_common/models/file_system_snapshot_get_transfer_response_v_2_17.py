...
from pypureclient.flashblade._common.models.error_context_response_errors_v_2_17 import ErrorContextResponseErrors
from pypureclient.flashblade._common.models.file_system_snapshot_transfer_v_2_17 import FileSystemSnapshotTransfer


class FileSystemSnapshotGetTransferResponse(BaseModel):
    """
    ...
    items: Optional[conlist(FileSystemSnapshotTransfer)] = Field(default=None, description="A list of file system snapshot objects.")
    ...
    total: Optional[FileSystemSnapshotTransfer] = Field(default=None, description="If `total_only` query param is `true`, then it will be a total of all records after filtering and no items will be returned. If `total_only` is `false`, then it will be a total of all records on this page.")
    __properties = ["continuation_token", "total_item_count", "items", "errors", "total"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FileSystemSnapshotGetTransferResponse:
        """Create an instance of FileSystemSnapshotGetTransferResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> FileSystemSnapshotGetTransferResponse:
        """Create an instance of FileSystemSnapshotGetTransferResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return FileSystemSnapshotGetTransferResponse.parse_obj(obj)

        _obj = FileSystemSnapshotGetTransferResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [FileSystemSnapshotTransfer.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "errors": [ErrorContextResponseErrors.from_dict(_item) for _item in obj.get("errors")] if obj.get("errors") is not None else None,
            "total": FileSystemSnapshotTransfer.from_dict(obj.get("total")) if obj.get("total") is not None else None
        })
        return _obj
