...
class NfsExportPolicyRuleBase(BaseModel):
    ...
    anongid: Optional[conint(strict=True, ge=0)] = Field(default=None, description="Any user whose GID is affected by an `access` of `root_squash` or `all_squash` will have their GID mapped to `anongid`. The default `anongid` is null, which means 65534. Use \"\" to clear.")
    anonuid: Optional[conint(strict=True, ge=0)] = Field(default=None, description="Any user whose UID is affected by an `access` of `root_squash` or `all_squash` will have their UID mapped to `anonuid`. The default `anonuid` is null, which means 65534. Use \"\" to clear.")
    ...
    __properties = ["id", "name", "access", "anongid", "anonuid", "atime", "client", "fileid_32bit", "permission", "policy", "policy_version", "required_transport_security", "secure", "security"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NfsExportPolicyRuleBase:
        """Create an instance of NfsExportPolicyRuleBase from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NfsExportPolicyRuleBase:
        """Create an instance of NfsExportPolicyRuleBase from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NfsExportPolicyRuleBase.parse_obj(obj)

        _obj = NfsExportPolicyRuleBase.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
            ...
            "required_transport_security": obj.get("required_transport_security"),
