...
from pypureclient.flashblade._common.models.network_interfaces_connectors_performance_v_2_24 import NetworkInterfacesConnectorsPerformance


class NetworkInterfacesConnectorsPerformanceGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(NetworkInterfacesConnectorsPerformance)] = Field(default=None, description="A list of network connector performance objects.")
    total: Optional[conlist(NetworkInterfacesConnectorsPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no individual items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NetworkInterfacesConnectorsPerformanceGetResponse:
        """Create an instance of NetworkInterfacesConnectorsPerformanceGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NetworkInterfacesConnectorsPerformanceGetResponse:
        """Create an instance of NetworkInterfacesConnectorsPerformanceGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NetworkInterfacesConnectorsPerformanceGetResponse.parse_obj(obj)

        _obj = NetworkInterfacesConnectorsPerformanceGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [NetworkInterfacesConnectorsPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [NetworkInterfacesConnectorsPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
