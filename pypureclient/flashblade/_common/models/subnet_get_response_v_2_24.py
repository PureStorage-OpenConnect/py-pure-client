...
from pypureclient.flashblade._common.models.subnet_v_2_24 import Subnet
...
class SubnetGetResponse(BaseModel):
    ...
    items: Optional[conlist(Subnet)] = Field(default=None, description="A list of subnet objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SubnetGetResponse:
        """Create an instance of SubnetGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> SubnetGetResponse:
        """Create an instance of SubnetGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SubnetGetResponse.parse_obj(obj)

        _obj = SubnetGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [Subnet.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
