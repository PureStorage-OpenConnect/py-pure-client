...
from pypureclient.flashblade._common.models.connection_relationship_performance_replication_v_2_24 import ConnectionRelationshipPerformanceReplication


class ConnectionRelationshipPerformanceReplicationGetResp(BaseModel):
    """
    ...
    items: Optional[conlist(ConnectionRelationshipPerformanceReplication)] = None
    total: Optional[conlist(ConnectionRelationshipPerformanceReplication)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ConnectionRelationshipPerformanceReplicationGetResp:
        """Create an instance of ConnectionRelationshipPerformanceReplicationGetResp from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> ConnectionRelationshipPerformanceReplicationGetResp:
        """Create an instance of ConnectionRelationshipPerformanceReplicationGetResp from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ConnectionRelationshipPerformanceReplicationGetResp.parse_obj(obj)

        _obj = ConnectionRelationshipPerformanceReplicationGetResp.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [ConnectionRelationshipPerformanceReplication.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [ConnectionRelationshipPerformanceReplication.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
