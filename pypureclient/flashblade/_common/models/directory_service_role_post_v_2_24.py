...
class DirectoryServiceRolePost(BaseModel):
    """
    ...
    management_access_policies: Optional[conlist(ReferenceWritable)] = Field(default=None, description="List of management access policies associated with the directory service role.")
    role: Optional[ReferenceWritable] = Field(default=None, description="Deprecated. `role` is deprecated in favor of `management_access_policies`, but remains for backwards compatibility. If a directory service role has exactly one access policy, which corresponds to a valid legacy role of the same name, `role` will be a reference to that role. Otherwise, it will be `null`.")
    __properties = ["group", "group_base", "management_access_policies", "role"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> DirectoryServiceRolePost:
        """Create an instance of DirectoryServiceRolePost from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> DirectoryServiceRolePost:
        """Create an instance of DirectoryServiceRolePost from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return DirectoryServiceRolePost.parse_obj(obj)

        _obj = DirectoryServiceRolePost.construct(_fields_set=None, **{
            "group": obj.get("group"),
            "group_base": obj.get("group_base"),
            "management_access_policies": [ReferenceWritable.from_dict(_item) for _item in obj.get("management_access_policies")] if obj.get("management_access_policies") is not None else None,
            "role": ReferenceWritable.from_dict(obj.get("role")) if obj.get("role") is not None else None
        })
        return _obj
