...
class DirectoryServiceManagement(BaseModel):
    ...
    """
    ssh_public_key_attribute: Optional[StrictStr] = Field(default=None, description="SSH public key attribute in the structure of the configured LDAP servers. The attribute field holds the user's SSH public key. This attribute can represent a single-valued or multi-valued field. If this attribute resolves to a multi-valued field, all valid SSH keys will be used. If the user has specified keys under Admin endpoints, these are used as well.")
