...
class ApiClient(BaseModel):
    ...
    access_policies: Optional[conlist(FixedReference)] = Field(default=None, description="The access policies allowed for ID Tokens issued by this API client. The bearer of an access token will be authorized to perform actions within the intersection of these policies and those of the array user specified as the JWT `sub` (subject) claim.")
    ...
    max_role: Optional[FixedReference] = Field(default=None, description="Deprecated. The maximum Admin Access Policy (previously 'role') allowed for ID Tokens issued by this API client. The bearer of an access token will be authorized to perform actions within the intersection of this policy and that of the array user specified as the JWT `sub` (subject) claim. `max_role` is deprecated in favor of `access_policies`, but remains for backwards compatibility. If a client has exactly one access policy which corresponds to a valid legacy role of the same name, `max_role` will be a reference to that role. Otherwise, it will be `null`.")
    ...
    __properties = ["id", "name", "access_policies", "access_token_ttl_in_ms", "enabled", "issuer", "key_id", "max_role", "public_key"]
    ...
    def from_dict(cls, obj: dict) -> ApiClient:
        ...
        _obj = ApiClient.construct(_fields_set=None, **{
            "id": obj.get("id"),
            "name": obj.get("name"),
            "access_policies": [FixedReference.from_dict(_item) for _item in obj.get("access_policies")] if obj.get("access_policies") is not None else None,
