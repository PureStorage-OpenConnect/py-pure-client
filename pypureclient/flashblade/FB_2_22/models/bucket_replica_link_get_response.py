...
from pypureclient.flashblade.FB_2_22.models.bucket_replica_link import BucketReplicaLink
from pypureclient.flashblade.FB_2_22.models.error_context_response_errors import ErrorContextResponseErrors


class BucketReplicaLinkGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(BucketReplicaLink)] = Field(default=None, description="A list of object store bucket replica link objects.")
    total: Optional[BucketReplicaLink] = Field(default=None, description="If `total_only` query param is `true`, then it will be a total of all records after filtering and no items will be returned. If `total_only` is `false`, then it will be a total of all records on this page.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BucketReplicaLinkGetResponse:
        """Create an instance of BucketReplicaLinkGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> BucketReplicaLinkGetResponse:
        """Create an instance of BucketReplicaLinkGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BucketReplicaLinkGetResponse.parse_obj(obj)

        _obj = BucketReplicaLinkGetResponse.construct(_fields_set=None, **{
            ...
            "items": [BucketReplicaLink.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": BucketReplicaLink.from_dict(obj.get("total")) if obj.get("total") is not None else None
        })
        return _obj
