...
from pypureclient.flashblade.FB_2_22.models.realm_space import RealmSpace
...
class RealmSpaceGetResponse(BaseModel):
    ...
    items: Optional[conlist(RealmSpace)] = Field(default=None, description="A list of realm space metrics objects.")
    total: Optional[conlist(RealmSpace)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    def from_dict(cls, obj: dict) -> RealmSpaceGetResponse:
        ...
        _obj = RealmSpaceGetResponse.construct(_fields_set=None, **{
            ...
            "items": [RealmSpace.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [RealmSpace.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
