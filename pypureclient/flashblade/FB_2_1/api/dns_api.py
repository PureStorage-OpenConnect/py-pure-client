...
from pypureclient.flashblade.FB_2_1.models.dns import Dns
from pypureclient.flashblade.FB_2_1.models.dns_get_response import DnsGetResponse
from pypureclient.flashblade.FB_2_1.models.dns_response import DnsResponse
...
class DNSApi:
    ...
    def api21_dns_get_with_http_info(
        self,
        ...
        **kwargs
        ...
        >>> thread = api.api21_dns_get_with_http_info(continuation_token, filter, ids, limit, names, offset, sort, async_req=True)
        >>> result = thread.get()
    ...
    def api21_dns_patch_with_http_info(
        self,
        dns: Dns,
        ids: Annotated[Optional[conlist(StrictStr)], Field(description="A comma-separated list of resource IDs. If after filtering, there is not at least one resource that matches each of the elements of `ids`, then an error is returned. This cannot be provided together with the `name` or `names` query parameters.")] = None,
        names: Annotated[Optional[conlist(StrictStr)], Field(description="A comma-separated list of resource names. If there is not at least one resource that matches each of the elements of `names`, then an error is returned.")] = None,
        **kwargs
        ...
        >>> thread = api.api21_dns_patch_with_http_info(dns, ids, names, async_req=True)
        >>> result = thread.get()
        ...
        for _key, _val in _params['kwargs'].items():
            if _key not in _all_params:
                raise ApiTypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method api21_dns_patch" % _key
                )
            _params[_key] = _val
        del _params['kwargs']
