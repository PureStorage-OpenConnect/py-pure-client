...
class SNMPManagersApi(object):
    ...
    def api211_snmp_managers_post_with_http_info(
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api211_snmp_managers_post_with_http_info(snmp_manager, names, async_req=True)
        >>> result = thread.get()
        ...
        if names is None:
            raise TypeError("Missing the required parameter `names` when calling `api211_snmp_managers_post`")
