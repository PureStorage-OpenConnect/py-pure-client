...
from pypureclient.flashblade.FB_2_20.models.realm_space_storage_class import RealmSpaceStorageClass


class RealmSpaceStorageClassGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(RealmSpaceStorageClass)] = Field(default=None, description="A list of realm space metrics objects for each storage class.")
    total: Optional[conlist(RealmSpaceStorageClass)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RealmSpaceStorageClassGetResponse:
        """Create an instance of RealmSpaceStorageClassGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> RealmSpaceStorageClassGetResponse:
        """Create an instance of RealmSpaceStorageClassGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return RealmSpaceStorageClassGetResponse.parse_obj(obj)

        _obj = RealmSpaceStorageClassGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [RealmSpaceStorageClass.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [RealmSpaceStorageClass.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
