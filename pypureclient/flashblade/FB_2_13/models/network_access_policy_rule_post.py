...
class NetworkAccessPolicyRulePost(object):
    """
    Attributes:
    ...
    def __setattr__(self, key, value):
        if key not in self.attribute_map:
            raise KeyError("Invalid key `{}` for `NetworkAccessPolicyRulePost`".format(key))
        ...
        for attr, _ in six.iteritems(self.swagger_types):
            if hasattr(self, attr):
                ...
                if isinstance(value, list):
                    ...
                    result[attr] = value
        if issubclass(NetworkAccessPolicyRulePost, dict):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, NetworkAccessPolicyRulePost):
            return False
