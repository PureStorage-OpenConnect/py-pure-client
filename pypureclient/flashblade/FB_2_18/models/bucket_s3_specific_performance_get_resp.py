...
from pypureclient.flashblade.FB_2_18.models.bucket_s3_specific_performance import BucketS3SpecificPerformance


class BucketS3SpecificPerformanceGetResp(BaseModel):
    """
    ...
    items: Optional[conlist(BucketS3SpecificPerformance)] = Field(default=None, description="A list of S3 specific bucket performance metrics objects.")
    total: Optional[conlist(BucketS3SpecificPerformance)] = Field(default=None, description="Total of all records after filtering. If `total_only` query param is `true`, then no items will be returned.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> BucketS3SpecificPerformanceGetResp:
        """Create an instance of BucketS3SpecificPerformanceGetResp from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> BucketS3SpecificPerformanceGetResp:
        """Create an instance of BucketS3SpecificPerformanceGetResp from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return BucketS3SpecificPerformanceGetResp.parse_obj(obj)

        _obj = BucketS3SpecificPerformanceGetResp.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [BucketS3SpecificPerformance.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None,
            "total": [BucketS3SpecificPerformance.from_dict(_item) for _item in obj.get("total")] if obj.get("total") is not None else None
        })
        return _obj
