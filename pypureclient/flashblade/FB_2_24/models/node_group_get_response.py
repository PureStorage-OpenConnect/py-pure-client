...
from pypureclient.flashblade.FB_2_24.models.node_group import NodeGroup


class NodeGroupGetResponse(BaseModel):
    """
    ...
    items: Optional[conlist(NodeGroup)] = Field(default=None, description="A list of node group objects.")
    ...
    @classmethod
    def from_json(cls, json_str: str) -> NodeGroupGetResponse:
        """Create an instance of NodeGroupGetResponse from a JSON string"""
        return cls.from_dict(json.loads(json_str))

    @classmethod
    def from_dict(cls, obj: dict) -> NodeGroupGetResponse:
        """Create an instance of NodeGroupGetResponse from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return NodeGroupGetResponse.parse_obj(obj)

        _obj = NodeGroupGetResponse.construct(_fields_set=None, **{
            "continuation_token": obj.get("continuation_token"),
            "total_item_count": obj.get("total_item_count"),
            "items": [NodeGroup.from_dict(_item) for _item in obj.get("items")] if obj.get("items") is not None else None
        })
        return _obj
