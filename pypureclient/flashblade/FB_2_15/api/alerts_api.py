...
class AlertsApi(object):
    ...
    def api215_alerts_get_with_http_info(
        ...
        if 'limit' in params and params['limit'] < 1:
            raise ValueError("Invalid value for parameter `limit` when calling `api215_alerts_get`, must be a value greater than or equal to `1`")
    ...
    def api215_alerts_patch_with_http_info(
        ...
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.api215_alerts_patch_with_http_info(alerts_settings, async_req=True)
        >>> result = thread.get()
        ...
        if alerts_settings is None:
            raise TypeError("Missing the required parameter `alerts_settings` when calling `api215_alerts_patch`")
