...
class LogsApi(object):
    ...
    def api20_logs_get_with_http_info(
        ...
        >>> thread = api.api20_logs_get_with_http_info(async_req=True)
        >>> result = thread.get()
        ...
        if 'end_time' in params and params['end_time'] < 0:
            raise ValueError("Invalid value for parameter `end_time` when calling `api20_logs_get`, must be a value greater than or equal to `0`")
        if 'start_time' in params and params['start_time'] < 0:
            raise ValueError("Invalid value for parameter `start_time` when calling `api20_logs_get`, must be a value greater than or equal to `0`")
        collection_formats = {}
        path_params = {}
